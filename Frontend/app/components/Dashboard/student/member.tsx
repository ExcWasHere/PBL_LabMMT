...
export default function StudentMemberPage() {
  ...
  const stats = useMemo(() => {
    const lecturer = memberList.filter((m) => 
      (m.field ?? "").toLowerCase() === "dosen" || 
      (m.position ?? "").toLowerCase() === "lecturer" || 
      (m.field ?? "").toLowerCase() === "lecturer"
    ).length;

    const student = memberList.filter((m) => 
      (m.field ?? "").toLowerCase() === "mahasiswa" || 
      (m.position ?? "").toLowerCase() === "student"
    ).length;

    const alumni = memberList.filter((m) => 
      (m.field ?? "").toLowerCase() === "alumni"
    ).length;
